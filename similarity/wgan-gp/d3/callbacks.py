...
class SaveImages(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        image_array = np.full((
            self.margin + (self.num_rows * (360 + self.margin)),
            self.margin + (self.num_cols * (360 + self.margin)), 3),
            255, dtype=np.uint8)
        ...
        for i in range(8):
            row = i // self.num_cols
            col = i % self.num_cols
            image_array[self.margin + (row * (360 + self.margin)):self.margin + (row * (360 + self.margin)) + 360,
                        self.margin + (col * (360 + self.margin)):self.margin + (col * (360 + self.margin)) + 360] = resized_stego_images[i]
