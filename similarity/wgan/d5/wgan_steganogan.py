...
class SteganoGAN(tf.keras.Model):
  def __init__(self, encoder=None, decoder=None, critic=None, data_depth=6, image_shape=(128, 128, 3), critic_extra_steps=5):
  ...
  def compile(self, encoder_decoder_optimizer, critic_optimizer, similarity_loss_fn, decoder_loss_fn):
    super(SteganoGAN, self).compile()
    self.encoder_decoder_optimizer = encoder_decoder_optimizer  or Adam(learning_rate=1e-4)
    self.critic_optimizer          = critic_optimizer           or Adam(learning_rate=1e-4, beta_1=0.5, beta_2=0.9)
