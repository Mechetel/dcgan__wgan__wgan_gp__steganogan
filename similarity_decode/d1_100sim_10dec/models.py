...
def Critic():
    ...
    scores = Conv2D(1, kernel_size=3, name='score_dense')(a) # (N, H/8, W/8, 128) -> (N, H/16, W/16, 1)
    global_avarage_score = GlobalAveragePooling2D(name='score_pooling')(scores) # (N, H/16, W/16, 1) -> (N, 1)

    return Model(inputs=Stego, outputs=global_avarage_score, name='ResnetSteganoGAN_critic')